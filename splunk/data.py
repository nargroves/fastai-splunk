...
OptSplunkTfms = Optional[Collection[SplunkProc]]
...
class SplunkLine(ItemBase):
    "Basic item for splunk line of data."
...
class SplunkProcessor(PreProcessor):
    ...
    def process_one(self, item):
        ...
        return SplunkLine(codes[0], conts[0], classes, col_names)
    ...
    def process(self, ds):
        ...
        for i,proc in enumerate(self.procs):
            if isinstance(proc, SplunkProc): proc(ds.inner_df, test=True)
            else:
...
class SplunkDataBunch(DataBunch):
    "Create a `DataBunch` suitable for splunk data."
    @classmethod
    def from_df(cls, path, df:DataFrame, dep_var:str, valid_idx:Collection[int], procs:OptSplunkTfms=None,
        ...
        procs = listify(procs)
        src = (SplunkList.from_df(df, path=path, cat_names=cat_names, cont_names=cont_names, procs=procs)
                           .split_by_idx(valid_idx))
        src = src.label_from_df(cols=dep_var) if classes is None else src.label_from_df(cols=dep_var, classes=classes)
        if test_df is not None: src.add_test(SplunkList.from_df(test_df, cat_names=cat_names, cont_names=cont_names,
...
class SplunkList(ItemList):
    "Basic `ItemList` for splunk data."
    _item_cls=SplunkLine
    _processor=SplunkProcessor
    _bunch=SplunkDataBunch
    def __init__(self, items:Iterator, cat_names:OptStrList=None, cont_names:OptStrList=None,
                 procs=None, **kwargs)->'SplunkList':
    ...
    @classmethod
    def from_splunk(cls, search_query, splunk_path, host, username, password, port=8088, cat_names:OptStrList=None, 
        cont_names:OptStrList=None, procs=None, **kwargs)->'ItemList':
        "Get the list of data returned by the splunk query in csv format"

        service = client.connect(host=host, port=port, username=username, password=password)
        search_query = search_query + ' | outputcsv create_empty=true override_if_empty=true singlefile=true results'
        job = service.jobs.create(search_query, **{"exec_mode": "blocking"})
        df = pd.read_csv(splunk_path + '/var/run/splunk/csv/results.csv')
        return cls(items=range(len(df)), cat_names=cat_names, cont_names=cont_names, procs=procs, inner_df=df.copy(), **kwargs)
